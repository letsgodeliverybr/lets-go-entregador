...
class _RotaDisponivelScreenState extends State<RotaDisponivelScreen> {
  ...
  Future<void> _aceitar() async {
    ...
    try {
      ...
      // Essa é a tela real onde a MAIORIA dos aceites acontece (tap no card
      // individual em pedidos_disponiveis_screen.dart, convite direto de
      // rota vindo de entregador_home_screen.dart) — usada tanto pra pedido
      // individual quanto pra rota agrupada (mesmo campo _pedido genérico
      // pros dois casos), então cancela os DOIS alertas possíveis; cancelar
      // um ID que nunca foi mostrado é no-op, sem risco.
