    ...
    try {
      switch (_etapa) {
        ...
        case EtapaEntrega.chegouDestino:
          ...
          // Pedido da integração real do iFood (origem=='ifood', com
          // ifood_order_id) — diferente do fluxo ifood_manual (WebView de
          // confirmacao-entrega-propria.ifood.com.br, pra loja sem API).
          // Aqui o código de 4 dígitos é o de ENTREGA que o iFood manda pro
          // cliente (evento DDCR) e precisa ser validado de volta na API
          // real (verifyDeliveryCode) antes de finalizar — sem isso o
          // iFood nunca marca o pedido CONCLUDED do lado dele. Busca
          // 'origem' fresco do banco (não confia em widget.pedido).
            ...
            if (_etapa == EtapaEntrega.retornando) ...[
              ...
              if (_etapa == EtapaEntrega.chegouDestino) ...[
                ...
                if (!comRetorno) ...[
                  ...
                  // "Novo Pedido" do painel.
                  //
                  // REVERTIDO (2026-09-17): tinha liberado esse botão pra
                  // origem=='ifood' também, mas a doc oficial de
                  // homologação (Cenário 8, critério obrigatório) exige o
                  // fluxo nativo — digitar o código no app e chamar
                  // verifyDeliveryCode direto — não essa WebView. NÃO usa
                  // origem=='ifood': esse campo é do pedido automático de
                  // verdade, que confirma a entrega pelo campo de 4
                  // dígitos em "Cheguei no destino" (ifood-validar-codigo),
                  // não por aqui. O travamento por telefone fictício no
                  // sandbox é limitação conhecida, aguardando resposta do
                  // suporte do iFood — não motivo pra trocar de mecanismo.
